...
| `gw4_board_locked_2026-09-10_early.csv` | 2026-09-10 13:52 UTC, deadline 2026-09-12 12:30 UTC | GW4 | **SECONDARY.** Second insurance lock, 46.6h out, on the board rebuilt after the 10 Sep stale-inputs audit (priors bit-identical; differs from this morning's board only at Brighton and Hull, from live availability). No GW4 team news existed yet. Includes `player_code`. |
| `gw4_board_locked_2026-09-11_deadline.csv` | 2026-09-11 11:31 UTC, deadline 2026-09-12 12:30 UTC | GW4 | **PRIMARY — pre-declared 2026-09-07, before any result was known.** Taken 25.0h out, not at kickoff, so it predates the last day of GW4 team news; the `_deadline` token makes it primary regardless. Lock time is the file mtime `[DERIVED]`, trusted because the 10 Sep lock's mtime matches the time recorded for it above. Includes `player_code`. |
| `gw5_board_locked_2026-09-18_deadline.csv` | 2026-09-18 07:11 UTC, deadline 2026-09-18 17:30 UTC | GW5 | **PRIMARY.** Built with the start prior UNCAPPED — it predates the same day's adoption of `INSEASON_KAPPA=4` (INTEGRATION_LOG, 17 Sep). No team news (`team_news_sources = 0`). |
| `gw5_board_locked_2026-09-18_kappa4.csv` | 2026-09-18 16:31 UTC, deadline 2026-09-18 17:30 UTC | GW5 | **SECONDARY, AND A DELIBERATE HEAD-TO-HEAD.** The same gameweek with `INSEASON_KAPPA=4`, the default adopted hours earlier. The two differ on 230 of 659 players (max +2.43 / −0.78, league total 963.8 → 1014.0) and on NOTHING ELSE: both carry `team_news_sources = 0`, so a difference in their scores is the cap, not a week of news. GW5 therefore scores the adoption on live data for free. `score_gw` takes the `_deadline` primary; score this one with `--prediction`. |
