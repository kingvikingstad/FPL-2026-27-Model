...
def prediction(gw):
    """The locked pre-deadline board if one exists, else today's board, flagged.

    A board written after the deadline is not a prediction and must never be reported as
    one; it is still worth scoring as a diagnostic, so this returns the provenance rather
    than refusing.
    """
    hits = sorted(glob.glob(_os.path.join(config.PREDICTIONS, f"gw{gw}_board_locked_*.csv")))
    if hits:
        return pd.read_csv(hits[-1]), f"locked: {_os.path.basename(hits[-1])}", True
    p = _os.path.join(config.OUTPUTS, "gw_board_long.csv")
    return pd.read_csv(p), "UNLOCKED current board (not a pre-deadline prediction)", False


def review(gw, force=False, write=True):
    ...
    act = actuals(gw)
    board, prov, locked = prediction(gw)
...
def selftest():
    ...
    print("SELFTEST OK: cup ties are excluded from the completeness test — both against "
          ...
          "league — and a part-played league round is not reported complete.")
