...
cal = calibrate_cold_start(hist_csv=config.COLDSTART_HIST)
...
proj.round(2).to_csv(os.path.join(config.OUTPUTS, "decision_gw1_6_full.csv"), index=False)
...
real.round(2).to_csv(os.path.join(config.OUTPUTS, "decision_gw1_6_realistic.csv"), index=False)
