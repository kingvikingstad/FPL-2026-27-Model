...
FINDING (from the validation run in this module's __main__): naive comparison
of congested vs rested weeks is CONFOUNDED — only the bigger clubs play European
football, so "congested" also proxies "better squad". The within-player fixed
effects comparison (same player, congested vs rested week) is the honest test;
see `fixed_effects()`. Net conclusion documented in RESOLVES_CHECKLIST.md /
project notes: the rotation effect on STARTS is small and mostly not
statistically distinguishable from noise once player identity is controlled for
— i.e. a "rotation null" result. Kept as a validated/documented null finding
rather than baked into the model as a strong prior.
"""
...
# ---------------------------------------------------------------------------
def validate(hist_csv="/mnt/user-data/uploads/fpl-data-stats.csv", season="2025-2026"):
